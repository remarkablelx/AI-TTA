  ...
  try {
    ...
    if (response.code === '0') {
      ...
      if (searchQuery.value) {
        filteredRecords = filteredRecords.filter((record: RecordItem) =>
      ...
      if (filterState.value !== null) {
        filteredRecords = filteredRecords.filter((record: RecordItem) => record.state === filterState.value);
      }

      // 应用排序
      filteredRecords.sort((a: RecordItem, b: RecordItem) => {
